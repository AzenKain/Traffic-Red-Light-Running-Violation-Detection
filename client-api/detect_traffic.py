...
def count_traffic_lights(img):
    ...
    result = results[0]  # Lấy phần tử đầu tiên
...
def draw_colored_lines(frame, color_name, lines):
    ...
    # Xác định màu dựa trên màu của đèn giao thông
    if color_name == "red":
        line_color = (0, 0, 255)  # BGR: Đỏ
        thickness = 3
    elif color_name == "yellow":
        line_color = (0, 255, 255)  # BGR: Vàng
        thickness = 3
    elif color_name == "green":
        line_color = (0, 255, 0)  # BGR: Xanh lá
        thickness = 3
    else:
        line_color = (255, 255, 255)  # BGR: Trắng
        thickness = 1
...
def determine_color(img, box, conf):
    ...
    frame = img.copy()  # Tạo bản sao của frame để không thay đổi frame gốc

    # Cắt phần đèn giao thông
    traffic_light = img[y1:y2, x1:x2]

    # Kiểm tra xem có hình ảnh được cắt thành công không
    if traffic_light.size == 0:
        ...
        # Định nghĩa các khoảng màu trong HSV
        ...
        min_pixel_threshold = max(50, total_area * 0.05)  # Tối thiểu 50 pixel hoặc 5% diện tích
    ...
    # Trả về frame đã được vẽ và tên màu
